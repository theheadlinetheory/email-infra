    ...
    if (action === "acquisition") {
      ...
      for (const cl of groupClients) {
        ...
        groups.push({
          ...
          health_score: avgHealth,
          flagged_domains: status === "warming" ? 0 : flaggedDomains.size,
          flagged_pct: status === "warming" ? 0 : (totalDomains ? Math.round(flaggedDomains.size / totalDomains * 100) : 0),
          needs_attention: status === "warming" ? false : (totalDomains ? flaggedDomains.size / totalDomains >= 0.15 : false),
          warmup_start: warmupStartStr,
          ready_date: readyDateStr,
          days_warming: daysWarming,
          days_left: daysLeft,
